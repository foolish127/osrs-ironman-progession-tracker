...
Done: the **Leagues** tab tracks the full Trailblazer Reloaded task list
(1,480 tasks, all 10 regions + General), with correct tier points (Easy 10,
Medium 40, Hard 80, Elite 200, Master 400 = 145,490 max). Generated by
`scripts/build_league_tasks.py`, which scrapes the wiki and conservatively
auto-completes tasks your data already proves (clues, levels, quests, boss KC,
owned items). Re-run it any time to refresh.

Remaining polish ideas:
- **Improve auto-complete recall** — current matching is precision-biased
  (~173 auto-done). Smarter item/boss name matching could catch more without
  false positives.
