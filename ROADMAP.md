...
### 1. Leagues-style master task checklist — all regions  *(✅ BUILT)*
Done: the **Leagues** tab tracks the full **Demonic Pacts (Leagues VI)** task
list (1,592 tasks, all 10 regions + General; Varlamore replaces Misthalin),
with correct tier points (Easy 10, Medium 30, Hard 80, Elite 200, Master 400 =
146,040 max). Generated by `scripts/build_league_tasks.py`, which scrapes the
wiki and conservatively auto-completes tasks your data already proves (clues,
levels, quests, achievement diaries, boss KC, owned items) — ~223 auto-done,
with a restriction guard so challenge variants ("under X min", "no food")
aren't wrongly marked. Re-run it any time to refresh.

Remaining polish ideas:
- **Improve auto-complete recall further** — many easy tasks are trivial
  actions (bury bones, cast a spell) that stats can't prove; matching could
  still catch more item/monster tasks without risking false positives.
- Mark tasks done by adding ` | done` (optionally a date) in
  `data/league_tasks.yaml`, or re-run the generator after your stats update.
